...
type CommentTag = 'revision' | 'comment' | 'annotation';
...
const SceneView: React.FC<SceneViewProps> = (props) => {
  ...
  const [selectedCommentType, setSelectedCommentType] = useState<CommentTag>('comment');
  ...
  const getFilteredComments = () => {
    let sceneComments = comments.filter(c => c.sceneId === currentScene);
    
    if (commentFilter !== 'all') {
      sceneComments = sceneComments.filter(c => c.type === commentFilter);
    }
  ...
  // 주석 저장 시 댓글로 자동 추가
  const handleAnnotationSave = (annotationData: string, comment: string) => {
    ...
    const annotationComment = {
      ...
      content: comment,
      time: '방금',
      type: 'annotation' as CommentTag,
      resolved: false,
      replies: [],
      annotationData: annotationData,
      imageType: imageViewMode
    };
    setComments([...comments, annotationComment]);
      ...
      <div className="flex-1 flex flex-col relative">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-gray-800">{currentSceneData.narration || '나레이션을 입력하세요'}</p>
                </div>
                <div>
                  <h4 className="font-bold text-sm text-gray-500 mb-2">장면 설명</h4>
                  <p className="text-gray-800">{currentSceneData.description || '장면 설명을 입력하세요'}</p>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-200">
                <h4 className="font-bold text-sm text-gray-500 mb-2">사운드</h4>
                <p className="text-gray-800">{currentSceneData.sound || '사운드 효과를 입력하세요'}</p>
              </div>
            </div>
                  ...
                  <div className="aspect-video bg-gray-100 flex items-center justify-center relative">
                    {currentSceneData.sketchUrl ? (
                      <>
                        ...
                        />
                        {/* 주석 오버레이 */}
                        {showAnnotationOverlay && filteredComments.find(c => 
                          c.id === showAnnotationOverlay && c.imageType === 'sketch' && c.annotationData
                        )?.annotationData && (
                          <div className="absolute inset-0 pointer-events-none">
                            <img 
                              src={filteredComments.find(c => c.id === showAnnotationOverlay)?.annotationData}
                              alt="초안 주석"
                              className="w-full h-full object-contain"
                            />
                          </div>
                  ...
                  <div className="aspect-video bg-gray-100 flex items-center justify-center relative">
                    {currentSceneData.artworkUrl ? (
                      <>
                        ...
                        {/* 주석 오버레이 */}
                        {showAnnotationOverlay && filteredComments.find(c => 
                          c.id === showAnnotationOverlay && c.imageType === 'artwork' && c.annotationData
                        )?.annotationData && (
                          <div className="absolute inset-0 pointer-events-none">
                            <img 
                              src={filteredComments.find(c => c.id === showAnnotationOverlay)?.annotationData}
        ...
        <div className="h-16 bg-white border-t border-gray-200 flex items-center justify-between px-6">
          ...
          <button 
            ...
            disabled={!storyboard.scenes || currentScene === storyboard.scenes.length - 1}
            className="flex items-center space-x-2 px-4 py-2 rounded hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span>다음 씬</span>
